...
impl<'a> CppCodeGenerator<'a> {
    ...
    fn generate_cpp_function_inner(
        ...
        if let Some(ret) = &details.return_conversion {
            let return_destiny = match placement_param {
                Some(placement_param) => Cow::Owned(format!("*{} =", placement_param)),
                None => Cow::Borrowed("return"),
            };
            underlying_function_call = format!(
                ...
                }.expect("Expected some conversion type for return value which resulted in a parameter name")
            );
        };
