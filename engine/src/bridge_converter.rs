...
impl<'a> BridgeConversion<'a> {
    ...
    fn convert_foreign_fn(
        ...
        let return_analysis = if is_constructor {
            let constructed_type = self_ty.as_ref().unwrap().to_bindgen_type_path();
            ReturnTypeAnalysis {
                rt: parse_quote! {
                    -> #constructed_type
                },
        ...
        F: FnOnce(Type) -> ArgumentConversion,
    {
        match ty {
            Type::Path(p) => {
                if self
                    .byvalue_checker
                    .is_pod(&TypeName::from_bindgen_type_path(p))
                {
    ...
    fn convert_type_path(
        ...
        if typ.path.segments.iter().next().unwrap().ident == "root" {
            ...
            let ty = TypeName::from_bindgen_type_path(&typ);
