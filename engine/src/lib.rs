...
mod tests {
    ...
    fn run_test(cxx_code: &str, header_code: &str, rust_code: TokenStream, allowed_funcs: &[&str]) {
        ...
        // our static library. Or, if we really need this nasty wrapper...
        // TODO - remove lstdc++ below. This is already on the command line
        // generated by rustc, but it's too early to be used to resolve symbols
        // missing from libautocxx-demo.a, so we need to specify it again at
        // the end.
        let wrapper_path = write_to_file(
            ...
            RUSTC=$1
            shift
            $RUSTC -C link-arg=-L$AUTOCXX_LIBRARY_PATH -C link-arg=-lautocxx-demo -C link-arg=-lstdc++ $@
            "},
        );
