...
impl FileLocationStrategy {
    ...
    pub fn make_include(&self, fname: String) -> TokenStream {
        match self {
            ...
            FileLocationStrategy::FromAutocxxRsFile(fname) => {
                let fname = fname.to_str().expect("AUTOCXX_RS_FILE environment variable contained non-UTF8 characters");
                quote! {
                    include!( #fname )
                }
            }
    ...
    fn get_gen_dir(&self, suffix: &str) -> PathBuf {
        let root = match self {
            ...
            FileLocationStrategy::FromAutocxxRsFile(_) => panic!("It's invalid to set AUTOCXX_RS_FILE during the codegen phase."),
        };
        root.join(suffix)
    }
